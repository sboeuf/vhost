...
impl Listener {
    ...
    pub fn accept(&self) -> Result<Option<UnixStream>> {
        match self.fd.accept() {
            Ok((socket, _addr)) => Ok(Some(socket)),
            Err(e) => {
...
impl<R: Req> Endpoint<R> {
    ...
    pub fn connect(path: &str) -> Result<UnixStream> {
        UnixStream::connect(path).map_err(Error::ConnectFail)
    }

    /// Create a master endpoint for a vhost-user connection.
    pub fn new_master(fd: UnixStream) -> Self {
        Endpoint {
            fd,
            master: true,
            _r: PhantomData,
    ...
    pub fn send_iovec(&mut self, iovs: &[&[u8]], fds: Option<&[RawFd]>) -> Result<usize> {
        ...
        self.fd.send_with_fds(iovs, rfds).map_err(|e| e.into())
    }
    ...
    pub fn recv_data(&mut self, len: usize) -> Result<(usize, Vec<u8>)> {
        ...
        let (bytes, _) = self.fd.recv_with_fds(&mut iovs, &mut [])?;
        Ok((bytes, rbuf))
    }
    ...
    pub fn recv_into_iovec(&mut self, iovs: &mut [iovec]) -> Result<(usize, Option<Vec<RawFd>>)> {
        ...
        let (bytes, fds) = self.fd.recv_with_fds(iovs, &mut fd_array)?;
        let rfds = match fds {
            0 => None,
            n => {
...
mod tests {
    ...
    fn send_data() {
        ...
        listener.set_nonblocking(true).unwrap();
        let sock = Endpoint::<MasterReq>::connect(UNIX_SOCKET_DATA).unwrap();
        let mut master = Endpoint::<MasterReq>::new_master(sock);
        let fd = listener.accept().unwrap().unwrap();
        let mut slave = Endpoint::<MasterReq>::new_slave(fd);
    ...
    fn send_fd() {
        ...
        listener.set_nonblocking(true).unwrap();
        let sock = Endpoint::<MasterReq>::connect(UNIX_SOCKET_FD).unwrap();
        let mut master = Endpoint::<MasterReq>::new_master(sock);
        let fd = listener.accept().unwrap().unwrap();
        let mut slave = Endpoint::<MasterReq>::new_slave(fd);
    ...
    fn send_recv() {
        ...
        listener.set_nonblocking(true).unwrap();
        let sock = Endpoint::<MasterReq>::connect(UNIX_SOCKET_SEND).unwrap();
        let mut master = Endpoint::<MasterReq>::new_master(sock);
        let fd = listener.accept().unwrap().unwrap();
        let mut slave = Endpoint::<MasterReq>::new_slave(fd);
