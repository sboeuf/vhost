...
use super::message::*;
use super::{Error, Result as VhostUserResult};
use crate::backend::{Result, VhostBackend, VhostUserMemoryRegionInfo, VringConfigData};

/// Vhost-user master side controller.
...
fn error_code<T>(err: Error) -> Result<T> {
    Err(crate::backend::Error::VhostUserProtocol(err))
}

/// Vhost-user master endpoint.
pub struct Master {
    //  Used to serialize calls to interface methods and accesses to internal master object.
    node: Mutex<MasterInternal>,
}

impl Master {
    /// Create a new instance.
    pub fn new(sock: UnixStream) -> Result<Self> {
        Ok(Master {
            node: Mutex::new(MasterInternal {
                fd: Endpoint::new_master(sock),
                virtio_features: 0,
    ...
    pub fn connect(path: &str) -> Result<Self> {
        Self::new(Endpoint::<MasterReq>::connect(path)?)
    }
}
...
impl VhostBackend for Master {
    ...
    fn set_mem_table(&mut self, regions: &[VhostUserMemoryRegionInfo]) -> Result<()> {
        if regions.is_empty() || regions.len() > MAX_ATTACHED_FD_ENTRIES {
            return error_code(Error::InvalidParam);
        }
...
impl VhostUserMaster for Master {
    fn get_protocol_features(&mut self) -> Result<VhostUserProtocolFeatures> {
        ...
        if node.virtio_features & flag == 0 || node.acked_virtio_features & flag == 0 {
            return error_code(Error::InvalidOperation);
        }
        ...
        match VhostUserProtocolFeatures::from_bits(node.protocol_features) {
            Some(val) => Ok(val),
            None => error_code(Error::InvalidContent),
        }
    }
    ...
    fn get_queue_num(&mut self) -> Result<u64> {
        ...
        if val.value > VHOST_USER_MAX_VRINGS {
            return error_code(Error::InvalidContent);
        }
    ...
    fn get_config(
        ...
        if rfds.is_some() {
            ...
            return error_code(Error::InvalidMessage);
        } else if reply.size == 0 {
            return error_code(Error::OperationFailedInSlave);
...
struct MasterInternal {
    fd: Endpoint<MasterReq>, // socket to send requests to slave.
    virtio_features: u64,
    ...
    max_queue_num: u64,
    failed: bool, // the endpoint is in failure mode
}
...
impl MasterInternal {
    ...
    fn send_fd_for_vring(
        ...
        if self.failed {
            return Err(Error::AlreadyClosed);
        } else if queue_index as u64 >= self.max_queue_num {
            return Err(Error::InvalidParam);
        }
        ...
        self.fd.send_message(&hdr, &msg, Some(&[fd]))?;
        Ok(hdr)
    }
    ...
    fn recv_reply<T: Sized + Default + VhostUserMsgValidator>(
        ...
        let (reply, body, rfds) = self.fd.recv_body::<T>()?;
    ...
    fn recv_reply_with_payload<T: Sized + Default + VhostUserMsgValidator>(
        ...
        let (reply, body, bytes, rfds) = self.fd.recv_payload_into_buf::<T>(&mut buf)?;
    ...
    fn wait_for_ack(&mut self, hdr: &VhostUserMsgHeader<MasterReq>) -> VhostUserResult<()> {
        ...
        let (reply, body, rfds) = self.fd.recv_body::<VhostUserU64>()?;
        if !reply.is_reply_for(&hdr) || rfds.is_some() || !body.is_valid() {
            Endpoint::<MasterReq>::close_rfds(rfds);
            return Err(Error::InvalidMessage);
        }
...
mod tests {
    ...
    fn create_pair(path: &str) -> (Master, Endpoint<MasterReq>) {
        ...
        (master, Endpoint::new_slave(slave))
    }
    ...
    fn create_master() {
        ...
        let mut slave = Endpoint::<MasterReq>::new_slave(listener.accept().unwrap().unwrap());
